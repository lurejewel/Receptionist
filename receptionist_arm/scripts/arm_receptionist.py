...
class arm_receptionist:
    ...
    def __init__(self):
        ...
        self.introducing_FinishSignal = False
        self.serving_FinishSignal = False

        rospy.spin()    

    def introducingCallback(self, msg):
    # Robot arm points at its right side after SPEECH part finishes its job
        if msg.data == True and self.introducing_FinishSignal == False: # manipulate only once
            # Manipulation...
            # Pointing at right side
            self.introducing_FinishSignal = True
            ...
            self.finish_pointing_pub.publish(self.finish)
            self.finish.data = False

    def servingCallback(self, msg):
    # Robot arm points forward after IMAGE part finishes its job
        if msg.data == True and self.serving_FinishSignal == False: # manipulate only once
            # Manipulation ...
            # Pointing forward
            self.serving_FinishSignal = True
    ...
    def back2init(self):
        ...
        rospy.loginfo('back to initial gesture of robot arm')
