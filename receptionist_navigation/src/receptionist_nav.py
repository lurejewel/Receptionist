...
class NavTest():
    def __init__(self):
        ...
        rospy.loginfo("*** Click the 2D Pose Estimate button in RViz to set the robot's initial pose...") # emmmm, 需要改一下，因为到时候initialpose已知了
        ...
        # -------------------------等待控制流唤醒---------------------------------------------
        rospy.Subscriber("/control",reception,self.control_callback,queue_size=1)
        ...
        while not rospy.is_shutdown():
            ...
            if self.initial_pose.header.stamp == "":
                distance = sqrt(pow(locations[self.location].position.x -
                                    locations[self.last_location].position.x, 2) +
                                pow(locations[self.location].position.y -
    ...
    def init_position(self):
        # 门口
        Pose_Door = Pose(Point(2.39777, 0.780624, 0.000), Quaternion(0.000, 0.000, -0.139022, 0.990289))
        self.set_goal("Door", Pose_Door)
        # 客厅
        Pose_Livingroom = Pose(Point(0.870332, 0.385404, 0.000), Quaternion(0.000, 0.000, 0.658435, 0.752638))
        self.set_goal("Livingroom", Pose_Livingroom)
        # 出口（可能没有）
