...
class speech_control(object):
    ...
    def __init__(self):
        ...
        self.sub_xfei_back_topic_name  = rospy.get_param("sub_xfei_back_topic_name" , "/xunfei_to_control")
    ...
    def xfeiCallback(self, msg):
        ...
        if msg.data.strip()=='':
            ...
            sh = self.sh
            voice = self.voice
            self.answer_question(sh,voice,output)


    def answer_question(self,sh,voice, output):
        if "joke" in output:
            """
                给他们讲个冷笑话...
            """
